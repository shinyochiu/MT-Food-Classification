...
def train_model(food_classifier, train_loader, valid_loader, test_loader, criterion, num_epochs=20):
    ...
    def valid(food_classifier, valid_loader, criterion):
        ...
        with open(arglist.data_dir + 'valid_{}_'.format(str(int(100*best_acc))) + '.csv', 'w', newline='') as outfile:
    ...
    def test(food_classifier, test_loader):
        print("testing")
        # food_classifier.load_checkpoint()
        # img_encoder.load_checkpoint()
        food_classifier.eval()
        # img_encoder.eval()
        with open(arglist.data_dir + 'test_{}_'.format(str(int(100*best_acc))) + '.csv', 'w', newline='') as outfile:
            writer = csv.writer(outfile)
            writer.writerow(['id', 'predicted'])
